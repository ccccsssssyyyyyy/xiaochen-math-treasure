...
> [!IMPORTANT]
> **数据库 schema 为本派生私有版本线（当前 `1010`），与上游不兼容。** 版本号规则是 `1000 + 上游版本号`：本派生从上游 v4 跳号到 `1004` 起自己的版本线，之后按自己的节奏推进（`1005` 错题三表 → `1006` 分栏列 → `1007` 人工合并列 → `1008` 题目学科 / 来源列 → `1009` 块图 URL 版本号清洗 → `1010` 错题分类列）。**程序只接受版本号不高于自己的库**：把上游 MathBank 的数据库直接换过来、或把本派生的库丢给上游程序，都会拒绝打开。如需引入上游题目，请走"题目导出 / 重新导入"链路，而非直接替换数据库文件。升级是自动的，且迁移前会先落一份带 SHA-256 的快照到 `data_backup/schema_snapshots/`。相关逻辑见 `mathbank/db_migrations.py`。
...
- 数据库迁移 `1004 → 1010`，迁移前自动落一份带校验和的快照。
...
升级到 2.4.0 后首次启动，程序会**自动**把数据库结构从 `1004` 升到 `1010`（新增错题三表与若干列）。迁移前会先落一份**迁移前快照**：

- 位置：`data_backup/schema_snapshots/`
- 文件名：`<数据库名>.schema-v1004-to-v1010.<UTC 时间戳>.db`，附同名 `.sha256` 校验和（权限 `600`）
